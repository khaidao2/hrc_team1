...
DEFAULT_URDF_PATH  = str(_REPO_ROOT / "assets" / "resources" / "s2.urdf")
DEFAULT_TASK_CFG   = str(_REPO_ROOT / "Ubtech_sim" / "config" / "Part_Sorting.yaml")
...
def _init_isaac_sim(task_cfg_path: str, headless: bool, prim_path: str, urdf_path: str):
    ...
    scene_usd = os.path.join(task_cfg.get("root_path", ""), task_cfg.get("scene_usd", ""))
    ...
    project_root = Path(task_cfg_path).parent.parent.parent
